...
# --- FIX IMPORTS: Import constants from config instead of processor ---
# from data_ingestion.processor import PROCESS_HISTORY_FILE, PROCESS_HISTORY_S3_KEY, AWS_S3_BUCKET_NAME
from data_ingestion.processor import LOCAL_PROCESS_HISTORY_FILE
from config import S3_HISTORY_KEY, S3_BUCKET_NAME, S3_PDF_PREFIX # Import necessary constants
# ---------------------------------------------------------------------
...
def manage_vector_stores(store_arg='all', cache_behavior='use'):
    """Orchestrates the two-phase data processing using the refactored DataProcessor."""

    start_time = time.time()
    send_status("start", {"message": "Processing run started.", "params": {"store": store_arg, "cache_behavior": cache_behavior}})

    logger.info(f"Starting vector store management. Store(s) arg: '{store_arg}', Cache Behavior: '{cache_behavior}'")
    logger.info(f"Using S3 PDF Prefix from config: {S3_PDF_PREFIX}")
    ...
    logger.info(f"Cache Behavior: {cache_behavior}")
    logger.info(f"S3 Prefix Used: {S3_PDF_PREFIX}")
...
def _reset_processing_history():
    """Reset processing history by deleting local and S3 files. Uses imported constants."""
    logger.info("Resetting processing history as requested")
    try:
        # Delete local file if it exists
        if os.path.exists(LOCAL_PROCESS_HISTORY_FILE):
            os.remove(LOCAL_PROCESS_HISTORY_FILE)
            logger.info(f"Deleted local {LOCAL_PROCESS_HISTORY_FILE}")
        else:
            logger.info(f"No existing local {LOCAL_PROCESS_HISTORY_FILE} to delete")
        ...
        if s3_client and S3_BUCKET_NAME:
            try:
                ...
                try:
                    s3_client.head_object(Bucket=S3_BUCKET_NAME, Key=S3_HISTORY_KEY)
                    # File exists, delete it
                    s3_client.delete_object(Bucket=S3_BUCKET_NAME, Key=S3_HISTORY_KEY)
                    logger.info(f"Deleted S3 process history: {S3_HISTORY_KEY}")
        ...
        logger.error(f"Error during process history reset: {e}")
...
if __name__ == "__main__":
    ...
    logger.info(f"Using S3 PDF Prefix from environment variable via config.")
