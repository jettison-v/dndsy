...
├── run_mobile_test_server.py# Utility to run dev server for mobile testing
├── data/                    # Data storage
...
*(Note: This section describes functionality integrated within `data_ingestion/processor.py`)*

The system includes a pipeline for generating rich metadata for each processed PDF document. This metadata enhances the Retrieval-Augmented Generation (RAG) process by providing additional context (like source title, summary, keywords, and categories) that can potentially be used for filtering or ranking search results in the future.

### Purpose

While not fully utilized in the current RAG retrieval logic, generating this metadata aims to:

1.  **Extract Key Information:** Pull out relevant details like title, summary, and keywords from the document content using an LLM.
2.  **Categorize Documents:** Assign categories based on both a predefined list (constrained, using `config.PREDEFINED_CATEGORIES`) and free-form analysis (automatic), both determined by an LLM.
3.  **Standardize Metadata:** Produce a consistent JSON structure for each document's metadata.
4.  **Store Metadata:** Upload the generated metadata JSON to a designated S3 location (`pdf-metadata/`) for potential later use.

### Integrated Functionality (`data_ingestion/processor.py`)

The metadata generation logic is part of the `DataProcessor` class:

1.  **Initialization:** Uses the same LLM provider configuration (`get_llm_client`) as the main application for consistency. Requires AWS S3 credentials for storing the output.
2.  **Orchestration (`_generate_and_upload_metadata`):** This internal function is called by `_preprocess_single_pdf` after text extraction.
    *   Generates a unique `document_id` (SHA256 hash of the S3 path).
    *   Calls internal helper functions (originally from `metadata_processor.py`) to generate summary, keywords, constrained category, automatic category, and extract the source title using the configured LLM.
    *   Uploads the resulting metadata dictionary to S3 (`s3://your-bucket/pdf-metadata/<document_id>.json`) via `upload_metadata_to_s3`.
3.  **LLM Calls:** Uses the configured LLM (`metadata_llm_client`) for:
    *   `_generate_metadata_summary`: Creates a brief summary.
    *   `_determine_metadata_constrained_category`: Classifies text against `PREDEFINED_CATEGORIES`.
    *   `_determine_metadata_automatic_category`: Generates a free-form category.
    *   `_extract_metadata_keywords`: Identifies keywords/phrases.
4.  **Current Status:** While the metadata is generated and stored, it's not currently factored into the vector search retrieval logic (`llm.py`). It exists for potential future enhancements.

### Metadata Schema

The generated metadata JSON stored in S3 follows this structure:
...
Relies on environment variables configured for the main application:

*   **S3 Configuration:** `AWS_S3_BUCKET_NAME`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`. Metadata is saved under the `pdf-metadata/` prefix.
*   **LLM Configuration:** `LLM_PROVIDER`, `LLM_MODEL_NAME`, provider-specific API keys (`OPENAI_API_KEY`, etc.).
